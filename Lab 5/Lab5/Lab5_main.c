...
uint16_t distanceInCm(void) {  //this function measures and returns the distance to the nearest object
    ...
    uint32_t pulseWidth = pulseIn()* 0.034 / 2; //calculate distance using s=t * 0.034/2. t comes from pulseIn() function
...
//from the ultrasonic sensor in terms of microseconds
{
    ...
    TA2CTL &= 0b1111111111001111;//stop the timer
...
void main(void)
{
    ...
    enum motor_states {FORWARD,BACKWARD,SWEEP_RIGHT,SWEEP_LEFT,TURN_RIGHT,TURN_LEFT} state, prevState;
	state = FORWARD;
	prevState = !FORWARD;
        ...
        switch (state) {
            ...
            case SWEEP_RIGHT:
                if (isNewState) {
                    stateTimer = 0;
                    Motor_Stop();
                    Servo(1999);  // Move servo to 90 right
                    Clock_Delay1ms(800);
            ...
            case SWEEP_LEFT:
                ...
                if (stateTimer >= 3) {
                    if (right_wall > left_wall){
                        state = TURN_RIGHT;// Pick the clearer path
                    }
